use ai_sdk::{AiAdapter, AiService, CloudflareAdapter, OllamaAdapter, OpenaiAdapter};
use chat_core::{
    ...
    fn from(mut agent: ChatAgent) -> Self {
        let adapter: AiAdapter = match agent.adapter {
            AdapterType::Openai => {
                ...
                let host = env::var("OPENAI_HOST").unwrap_or("https://vip.apiyi.com/v1".to_string());
                OpenaiAdapter::new(host, api_key, agent.model).into()
            }
            ...
            AdapterType::Cloudflare => {
                ...
                let account = env::var("CF_API_ACCOUNT").expect("CF_API_ACCOUNT not set");
                CloudflareAdapter::new(api_key, account, agent.model).into()
            }
        };
        match agent.r#type {
...
mod tests {
    ...
    async fn agent_variant_should_work() -> Result<()> {
        ...
        if let AgentDecision::Modify(_content) = decision {} else {
